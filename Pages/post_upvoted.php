...
<html>
    ...
    <body>
		<header>
        <h1 id="title">Post from Most Upvoted</h1>
        <h4 class="logsign">
            <?php
            session_start();
        ...
        <main>
			<content>
                    ...
                    <?php
                        ...
                        $star=0;
                        $pos=0;
                        $result=mysqli_query($dbconn, "SELECT j.id_joke,email , title, text_joke,CAST(AVG(vote)as decimal (10,1)) as avg FROM users u inner join (jokes j left join votes v on j.id_joke=v.id_joke) on u.id_user=j.id_user where shows=true group by email order by avg desc limit 6;");
